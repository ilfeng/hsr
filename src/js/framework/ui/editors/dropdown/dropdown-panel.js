...
define(['jquery', 'core', 'ui-panel', 'ui-tool-manager'], function ($, hsr, _super, ToolUtils) {
    ...
    var metedata = {
         ...
         * @readonly
         */
        cssClass: '@CSS_PREFIX@editor-dropdown-panel',
        /**
         * 名称。
         * @type {string}
         * @readonly
    ...
    // 注册组件（基类）。
    ToolUtils.regiestBaseClass(DropdownPanelClass);
    ...
    $.extend(DropdownPanelClass.prototype, metedata, {
        show: function () {
            ...
            // 绑定事件。
            $([window.top, window]).on('mousedown.editor@EVENT_POSTFIX@ resize.editor@EVENT_POSTFIX@', function () {
                self.hide();
                return false;
            });
        },
        hide: function () {
            ...
            // 移除所有事件。
            $([window.top, window]).off('.editor@EVENT_POSTFIX@');
        },
        /**
         * 切换面板显示状态。
         */
        ...
        _reckonPosition: function () {
            var self = this, panel$ = self._element$, editor$ = self._options.editor,
                offset = editor$.offset(), editorHeight = editor$.height(), editorWidth = editor$.width(),
